...
public final class Utility {
	...
	public static String GetPOSString(int nPOS) {
		...
		if (nPOS > (int) ('a') * 25600) {
			if ((nPOS / 100) % 256 != 0) {
				sPOSRet = String.format("%c%c+%c", (nPOS / 25600),
						((nPOS / 100) % 256), nPOS % 100);
			} else {
				sPOSRet = String.format("%c+%c", (nPOS / 25600), nPOS % 100);
			}
		} else {
			if (nPOS > 256) {
				sPOSRet = String.format("%c%c", (nPOS / 256), (nPOS % 256));
			} else {
				sPOSRet = String.format("%c", (nPOS % 256));
			}
		}
		return sPOSRet;
	}
	...
	// 根据汉字的两个字节返回对应的CC_ID
	// ====================================================================
	public static int CC_ID(byte b1_, byte b2_) {
		int b1 =  getUnsigned(b1_);
		int b2 =  getUnsigned(b2_);
		return ((int) (b1) - 176) * 94 + ((int) (b2) - 161);
	}
	...
	public static int CC_ID(char c) {
		...
		//byte b1 = (byte) getUnsigned(b1_);
		//byte b2 = (byte) getUnsigned(b2_);
		
		if (b.length != 2) {
			return -1;
		} else {
			return ((int) (getUnsigned(b[0])) - 176) * 94 + ((int) (getUnsigned(b[1])) - 161);
		}
	}
	...
	public static int charType(char c) {
		...
		// byte[] byteArray = Encoding.GetEncoding("gb2312").GetBytes((new
		// Character(c)).toString());

		byte[] byteArray = null;
		try {
			byteArray = String.valueOf(c).getBytes("gb2312");
		} catch (UnsupportedEncodingException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		byte b1 = byteArray[0];
		byte b2 = byteArray.length > 1 ? byteArray[1] : 0;

		if (byteArray.length > 2) {
			return Predefine.CT_OTHER;
		}

		return DoubleByteCharType(getUnsigned(b1), getUnsigned(b2));
	}
	...
	private static int DoubleByteCharType(int b1, int b2) {
		...
		if (b1 < 128) {
			...
			if (delimiters.indexOf((char) (b1)) >= 0) {
	...
	// ====================================================================
	public static boolean IsAllChinese(String sString) {
		byte[] byteArray = String2ByteArray(sString);

		int nLen = byteArray.length, i = 0;

		while (i < nLen - 1 && (int) (byteArray[i]) < 248
				&& (int) (byteArray[i]) > 175) {
			i += 2;
		}
		if (i < nLen) {
	...
	// ====================================================================
	public static boolean IsAllNum(String sString) {

		Pattern p = Pattern.compile(
				"^[±+－\\-＋]?[０１２３４５６７８９\\d]*[∶·．／./]?[０１２３４５６７８９\\d]*[百千万亿佰仟％‰%]?$");

		return p.matcher(sString).matches();

		// return
		// Regex.IsMatch(sString,"^[±+－\\-＋]?[０１２３４５６７８９\\d]*[∶·．／./]?[０１２３４５６７８９\\d]*[百千万亿佰仟％‰%]?$");
	}

	// ====================================================================
	// Decide whether the word is Chinese Num word
	// ====================================================================
	public static boolean IsAllChineseNum(String sWord) {
		// 百分之五点六的人早上八点十八分起床
		return Regex.IsMatch(sWord,
				"^[几数第上成]?[零○一二两三四五六七八九十廿百千万亿壹贰叁肆伍陆柒捌玖拾佰仟∶·．／点]*[分之]?[零○一二两三四五六七八九十廿百千万亿壹贰叁肆伍陆柒捌玖拾佰仟]*$");
	}
	...
	// ====================================================================
	public static boolean IsAllLetter(String sString) {
	...
	public static int GetForeignCharCount(String sWord) {
		...
		nForeignCount = GetCharCount(Predefine.TRANS_ENGLISH, sWord); // English
																		// char
																		// counnts
		nCount = GetCharCount(Predefine.TRANS_JAPANESE, sWord); // Japan char
																// counnts
		if (nForeignCount <= nCount) {
			nForeignCount = nCount;
		}
		nCount = GetCharCount(Predefine.TRANS_RUSSIAN, sWord); // Russian char
																// counnts
		if (nForeignCount <= nCount) {
			nForeignCount = nCount;
		}
	...
	public static int CCStringCompare(String str1, String str2) {
		...
		for (int i = 0; i < minLength; i++) {
			if ((int) (ca1[i]) < 128 && (int) (ca2[i]) < 128) // 如果两个字符都是半角
			{
				if (ca1[i] < ca2[i]) {
					return -1;
				} else if (ca1[i] > ca2[i]) {
			...
			} else if ((int) (ca1[i]) < 128) {
				return -1;
			} else if ((int) (ca2[i]) < 128) {
				return 1;
			} else // 两个字符全部是全角
			{
	...
	// ====================================================================
	// public static boolean GB2312_Generate(String sFileName) {
	...
	// ====================================================================
	// public static boolean CC_Generate(String sFileName) {
	// boolean isSuccess = true;
	// byte[] b = new byte[2];
	// FileStream outputFile = null;
	// StreamWriter writer = null;
	...
	// for (int j = 161; j < 255; j++) {
	// b[0] = Byte.parseByte(i);
	// b[1] = Byte.parseByte(j);
	// writer.WriteLine(String.format("%1$s, %2$s, %3$s",
	// ByteArray2String(b), i, j));
	// }
	// }
	// } catch (java.lang.Exception e) {
	...
	public static String Traditional2Simplified(String s) {
		...
		for (char c : sArray) {
			if (trad.indexOf(c) >= 0) {
				sb.append(simp.substring(trad.indexOf(c), trad.indexOf(c) + 1));
			} else {
				sb.append(c);
			}
		}
	...
	//
	// //Get the postfix
	// public static bool PostfixSplit(string sWord, string sWordRet, string
	// sPostfix)
	// {
	// return true;
	// }
