...
import nlp.cws.utility.ContextStat;
...
public class Span {
	...
	private TAG_TYPE m_tagType = TAG_TYPE.forValue(0); // The type of tagging
	...
	public final boolean LoadContext(String sFilename) {
	...
	public final boolean PersonRecognize(WordDictionary personDict) {
		...
		double[] dFactor = {0.003606, 0.000021, 0.001314, 0.000315, 0.656624,
				0.000021, 0.146116, 0.009136, 0.000042, 0.038971, 0, 0.090367,
		...
		for (i = 1; m_nBestTag[i] > -1; i++) {
			sb.append((char) (m_nBestTag[i] + (int) ('A')));
		}
		...
		while (j < i) {
			...
			for (k = 0; !bMatched && nPatternLen[k] > 0; k++) {
				// if (String.Compare(sPatterns[k], 0, sPOS, j,
				// nPatternLen[k]) == 0
				// && m_sWords[j - 1].compareTo("·") != 0
				// && m_sWords[j + nPatternLen[k]].compareTo("·") != 0) {

				// System.out.println(sPatterns[k].substring(0,
				// nPatternLen[k]));
				// System.out.println(sPOS.substring(j, j + nPatternLen[k]));
				// System.out.println(m_sWords[j - 1]);
				// System.out.println(m_sWords[j + nPatternLen[k]]);
				if (j + nPatternLen[k] <= sPOS.length()
						&& sPatterns[k].substring(0, nPatternLen[k])
								.equalsIgnoreCase(
										sPOS.substring(j, j + nPatternLen[k]))
						&& !m_sWords[j - 1].equalsIgnoreCase("·")
						&& !m_sWords[j + nPatternLen[k]]
								.equalsIgnoreCase("·")) {

					// Find the proper pattern k
					if (sPatterns[k].compareTo("FB") == 0
							&& (sPOS.charAt(j + 2) == 'E'
									|| sPOS.charAt(j + 2) == 'C'
									|| sPOS.charAt(j + 2) == 'G')) {
					...
					while (nPos < j + nPatternLen[k]) {
						...
						//
						if (m_nBestTag[nPos] < 4 && personDict.GetFrequency(
								m_sWords[nPos],
								m_nBestTag[nPos]) < Predefine.LITTLE_FREQUENCY) {
							nLittleFreqCount++;
						}
					...
					// }
					//
					if (sPatterns[k].compareTo("CDCD") == 0) {
						// Rule for exclusion
					...
					// //The all roles appear with two lower frequecy,we will
					// ignore them
					// continue;
					//
					m_nUnknownWords[m_nUnknownWordsCount][0] = m_nWordPosition[j];
					m_nUnknownWords[m_nUnknownWordsCount][1] = m_nWordPosition[j
							+ nPatternLen[k]];
					m_dWordsPossibility[m_nUnknownWordsCount] = -Math
							.log(dFactor[k])
	...
	private boolean GuessPOS(int nIndex, RefObject<Integer> pSubIndex) {
		...
		switch (m_tagType) {
			...
			case TT_PERSON :
				j = 0;
				if ((new String("××")).indexOf(m_sWords[nIndex]) != -1) {
					m_nTags[i][j] = 6;
					m_dFrequency[i][j++] = 1.0
							/ (m_context.GetFrequency(0, 6) + 1);
				} else {
					m_nTags[i][j] = 0;
					m_dFrequency[i][j++] = 1.0
							/ (m_context.GetFrequency(0, 0) + 1);
					...
					if (nLen >= 2) {
						...
						m_nTags[i][j] = 11;
						m_dFrequency[i][j++] = 1.0
								/ (m_context.GetFrequency(0, 11) * 8);
						m_nTags[i][j] = 12;
						m_dFrequency[i][j++] = 1.0
								/ (m_context.GetFrequency(0, 12) * 8);
						m_nTags[i][j] = 13;
						m_dFrequency[i][j++] = 1.0
								/ (m_context.GetFrequency(0, 13) * 8);
					} else if (nLen == 1) {
						m_nTags[i][j] = 0;
						m_dFrequency[i][j++] = 1.0
								/ (m_context.GetFrequency(0, 0) + 1);
						nCharType = Utility
								.charType(m_sWords[nIndex].toCharArray()[0]);
						if (nCharType == Predefine.CT_OTHER
								|| nCharType == Predefine.CT_CHINESE) {
							m_nTags[i][j] = 1;
							m_dFrequency[i][j++] = 1.0
									/ (m_context.GetFrequency(0, 1) + 1);
							m_nTags[i][j] = 2;
							m_dFrequency[i][j++] = 1.0
									/ (m_context.GetFrequency(0, 2) + 1);
							m_nTags[i][j] = 3;
							m_dFrequency[i][j++] = 1.0
									/ (m_context.GetFrequency(0, 3) + 1);
							m_nTags[i][j] = 4;
							m_dFrequency[i][j++] = 1.0
									/ (m_context.GetFrequency(0, 4) + 1);
						}
						m_nTags[i][j] = 11;
						m_dFrequency[i][j++] = 1.0
								/ (m_context.GetFrequency(0, 11) * 8);
			...
			case TT_TRANS_PERSON :
				...
				if (!Utility.IsAllChinese(m_sWords[nIndex])) {
					if (Utility.IsAllLetter(m_sWords[nIndex])) {
						...
						m_nTags[i][j] = 11;
						m_dFrequency[i][j++] = 1.0
								/ (m_context.GetFrequency(0, 11) + 1);
						m_nTags[i][j] = 2;
						m_dFrequency[i][j++] = 1.0
								/ (m_context.GetFrequency(0, 2) * 2 + 1);
						m_nTags[i][j] = 3;
						m_dFrequency[i][j++] = 1.0
								/ (m_context.GetFrequency(0, 3) * 2 + 1);
						m_nTags[i][j] = 12;
						m_dFrequency[i][j++] = 1.0
								/ (m_context.GetFrequency(0, 12) * 2 + 1);
						m_nTags[i][j] = 13;
						m_dFrequency[i][j++] = 1.0
								/ (m_context.GetFrequency(0, 13) * 2 + 1);
					}
					m_nTags[i][j] = 41;
					m_dFrequency[i][j++] = 1.0
							/ (m_context.GetFrequency(0, 41) * 8);
					m_nTags[i][j] = 42;
					m_dFrequency[i][j++] = 1.0
							/ (m_context.GetFrequency(0, 42) * 8);
					m_nTags[i][j] = 43;
					m_dFrequency[i][j++] = 1.0
							/ (m_context.GetFrequency(0, 43) * 8);
				} else if (nLen >= 2) {
					m_nTags[i][j] = 41;
					m_dFrequency[i][j++] = 1.0
							/ (m_context.GetFrequency(0, 41) * 8);
					...
					if (nCharType == Predefine.CT_OTHER
						...
						m_nTags[i][j] = 1;
						m_dFrequency[i][j++] = 1.0
								/ (m_context.GetFrequency(0, 1) * 2 + 1);
						m_nTags[i][j] = 2;
						m_dFrequency[i][j++] = 1.0
								/ (m_context.GetFrequency(0, 2) * 2 + 1);
						m_nTags[i][j] = 3;
						...
						m_nTags[i][j] = 30;
						m_dFrequency[i][j++] = 1.0
								/ (m_context.GetFrequency(0, 30) * 8 + 1);
						m_nTags[i][j] = 11;
						m_dFrequency[i][j++] = 1.0
								/ (m_context.GetFrequency(0, 11) * 4 + 1);
						m_nTags[i][j] = 12;
						m_dFrequency[i][j++] = 1.0
								/ (m_context.GetFrequency(0, 12) * 4 + 1);
						m_nTags[i][j] = 13;
						m_dFrequency[i][j++] = 1.0
								/ (m_context.GetFrequency(0, 13) * 4 + 1);
						m_nTags[i][j] = 21;
						m_dFrequency[i][j++] = 1.0
								/ (m_context.GetFrequency(0, 21) * 2 + 1);
						m_nTags[i][j] = 22;
						m_dFrequency[i][j++] = 1.0
								/ (m_context.GetFrequency(0, 22) * 2 + 1);
						m_nTags[i][j] = 23;
						m_dFrequency[i][j++] = 1.0
								/ (m_context.GetFrequency(0, 23) * 2 + 1);
					}
					m_nTags[i][j] = 41;
					m_dFrequency[i][j++] = 1.0
							/ (m_context.GetFrequency(0, 41) * 8);
	...
	private double ComputePossibility(int nStartPos, int nLength,
		...
		for (int i = nStartPos; i < nStartPos + nLength; i++) {
			...
			dPOSPoss = Math.log(
					(double) (m_context.GetFrequency(0, m_nBestTag[i]) + 1))
					- Math.log((double) (nFreq + 1));
			dRetValue += dPOSPoss;
			//
	...
	public final boolean PlaceRecognize(WordDictionary dictCore,
			WordDictionary placeDict) {
		...
		while (m_nBestTag[i] > -1) {
			...
			if (nEnd > nStart) {
				...
				m_dWordsPossibility[m_nUnknownWordsCount++] = ComputePossibility(
	...
	public final void ReleaseSpan() {
		m_context.ReleaseContextStat();
	}

	// Set the tag type
	public final void SetTagType(TAG_TYPE nType) {
		m_tagType = nType;
	}

	public final void SetTagType() {
		SetTagType(TAG_TYPE.TT_NORMAL);
	}

	// POS tagging with Hidden Markov Model
	public final boolean POSTagging(WordResult[] pWordItems,
			WordDictionary dictCore, WordDictionary dictUnknown) {
			...
			switch (m_tagType) {
				case TT_NORMAL :
					...
					while (m_nBestTag[j] != -1 && j < m_nCurLength) {
						...
						// Let 。be 0
						if (pWordItems[j + nStartPos - 1].dValue > 0 && dictCore
								.IsExist(pWordItems[j + nStartPos - 1].sWord,
										-1))
						// Exist and update its frequncy as a POS value
						{
							pWordItems[j + nStartPos - 1].dValue = dictCore
									.GetFrequency(
											pWordItems[j + nStartPos - 1].sWord,
											m_nBestTag[j]);
						}
						j += 1;
					}
	...
	private int GetFrom(WordResult[] pWordItems, int nIndex,
			WordDictionary dictCore, WordDictionary dictUnknown) {
		WordInfo info;
		@SuppressWarnings("unused")
		...
		for (i = 1; i < Predefine.MAX_WORDS_PER_SENTENCE
			...
			if (m_tagType == TAG_TYPE.TT_NORMAL || !dictUnknown
					.IsExist(pWordItems[nWordsIndex].sWord, 44)) {
				m_sWords[i] = pWordItems[nWordsIndex].sWord; // store current
																// word
			...
			if (m_tagType != TAG_TYPE.TT_NORMAL) {
				...
				if (m_tagType == TAG_TYPE.TT_TRANS_PERSON && i > 0
						&& m_sWords[i - 1] != null
						&& Utility.charType(m_sWords[i - 1]
								.toCharArray()[0]) == Predefine.CT_CHINESE) {
					if (m_sWords[i].equals(".")) {
						sCurWord = "．";
					} else if (m_sWords[i].equals("-")) {
						sCurWord = "－";
					}
				}
				...
				if (info != null) {
					...
					for (j = 0; j < info.Count; j++) {
						...
						m_nTags[i][j] = info.POSs.get(j);
						m_dFrequency[i][j] = -Math
								.log((double) (1 + info.Frequencies.get(j)))
								+ Math.log((double) (m_context.GetFrequency(0,
										info.POSs.get(j)) + nPOSCount));
					}
				} else {
					nPOSCount = 1;
					j = 0;
				...
				if (m_sWords[i].compareTo("始##始") == 0) {
					m_nTags[i][j] = 100;
					m_dFrequency[i][j] = 0;
					j++;
				} else if (m_sWords[i].compareTo("末##末") == 0) {
					m_nTags[i][j] = 101;
					m_dFrequency[i][j] = 0;
					j++;
				} else {
			...
			// For normal POS tagging
			{
				...
				if (pWordItems[nWordsIndex].nPOS > 0) {
					...
					m_dFrequency[i][j] = -Math
							.log(pWordItems[nWordsIndex].dValue)
							+ Math.log((double) (m_context.GetFrequency(0,
									m_nTags[i][j]) + 1));
					...
					if (info != null) {
						...
						for (; j < info.Count; j++) {
							// Get the POS set of sCurWord in the unknown
							// dictionary
							m_nTags[i][j] = info.POSs.get(j);
							m_dFrequency[i][j] = -Math
									.log(1 + info.Frequencies.get(j))
			...
			if (j == 0) {
				// We donot know the POS, so we have to guess them according
				// lexical knowledge
				RefObject<Integer> tempRef_j = new RefObject<Integer>(j);
				GuessPOS(i, tempRef_j); // Guess the POS of current word
